...
#pragma unroll COLOR_DIM
    ...
    if (inside) {
        ...
        // We store double precision as T would be used in backward pass and
        // it can be very small and causing large diff in gradients with float32.
        render_alphas[pix_id] = 1.0 - T;
#pragma unroll COLOR_DIM
        for (int k = 0; k < COLOR_DIM; ++k) {
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> rasterize_to_pixels_fwd_tensor(
    ...
    torch::Tensor alphas = torch::empty({C, image_height, image_width, 1},
                                        means2d.options().dtype(torch::kFloat64));
...
__global__ void rasterize_to_pixels_bwd_kernel(
    ...
    double T_final = 1.0 - render_alphas[pix_id];
    double T = T_final;
...
#pragma unroll COLOR_DIM
        ...
        for (int t = max(0, batch_end - warp_bin_final); t < batch_size; ++t) {
            ...
            if (valid) {
                ...
                double ra = 1.0 / (1.0 - static_cast<double>(alpha));
                T *= ra;
...
#pragma unroll COLOR_DIM
                ...
                // v_alpha += -T_final * ra * background.x * v_render_c.x;
                // v_alpha += -T_final * ra * background.y * v_render_c.y;
                // v_alpha += -T_final * ra * background.z * v_render_c.z;
